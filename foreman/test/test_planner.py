...
def test_scenario_1_standard_bring_up(basic_planner):
    state = SystemState(
        components={
            "franka_hw": Component(
                "franka_hw", ComponentType.HARDWARE, LifecycleState.UNCONFIGURED
            ),
            "franka_jtc": Component(
                "franka_jtc", ComponentType.CONTROLLER, LifecycleState.UNCONFIGURED
            ),
        }
    )
    goal = SystemGoal(
...
def test_scenario_3_broadcaster_bringup(broadcaster_planner):
    state = SystemState(
        components={
            ...
            "joint_state_broadcaster": Component(
                "joint_state_broadcaster", ComponentType.CONTROLLER, LifecycleState.UNCONFIGURED
            ),
        }
    )
    goal = SystemGoal(
...
def test_scenario_6_hardware_failure(basic_planner):
    state = SystemState(
        components={
            ...
            "franka_jtc": Component(
                "franka_jtc", ComponentType.CONTROLLER, LifecycleState.INACTIVE
            ),
        }
    )
    goal = SystemGoal(
...
def test_scenario_9_asymmetric_hardware_failure(asymmetric_planner):
    state = SystemState(
        components={
            ...
            "dual_arm_jtc": Component(
                "dual_arm_jtc", ComponentType.CONTROLLER, LifecycleState.INACTIVE
            ),
        }
    )
    goal = SystemGoal(
...
def test_scenario_11_lifecycle_node_bringup(lifecycle_planner):
    ...
    state = SystemState(
        components={
            "robot_manager": Component(
                "robot_manager", ComponentType.LIFECYCLE_NODE, LifecycleState.UNCONFIGURED
            ),
...
def test_scenario_12_controller_blocked_by_lifecycle_node(lifecycle_planner):
    ...
    state = SystemState(
        components={
            "robot_manager": Component(
                "robot_manager", ComponentType.LIFECYCLE_NODE, LifecycleState.INACTIVE
            ),
...
def test_scenario_13_lifecycle_node_stepdown_blocked_by_controller(lifecycle_planner):
    ...
    state = SystemState(
        components={
            "robot_manager": Component(
                "robot_manager", ComponentType.LIFECYCLE_NODE, LifecycleState.ACTIVE
            ),
...
def test_scenario_14_mixed_hw_lifecycle_and_controllers():
    ...
    infra_indices = [
        i for i, (name, _) in enumerate(transitions) if name in ("hw_arm", "robot_manager")
    ]
